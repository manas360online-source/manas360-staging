...
const ModuleLink = ({ id, title, active, locked, completed, onClick }: { id: string, title: string, active: boolean, locked: boolean, completed: boolean, onClick: () => void }) => (
    <button
        onClick={onClick}
        disabled={locked}
        className={`block w-full text-left px-4 py-2.5 text-sm rounded-lg transition-all flex items-center justify-between group ${active
            ? 'bg-[#EBF5FF] text-[#0066FF] font-bold border-l-[3px] border-[#0066FF]'
            : locked
...
const ModuleSection = ({
    ...
    const tag = titleParts[0].trim();
    const heading = titleParts[1]?.trim() || '';

    return (
        <section id={id} className="scroll-mt-8 relative mb-16">
            {isLocked && <LockOverlay />}
            <div className="flex items-center gap-3 mb-6">
                <span className="bg-[#EBF5FF] text-[#0066FF] text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                    {tag}
                </span>
                <h2 className="text-xl font-bold text-[#1A2332]">{heading}</h2>
                {progress === 100 && <CheckCircle className="ml-2 w-5 h-5 text-green-500" />}
            </div>
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 text-center">
                <h3 className="font-bold text-blue-900 mb-2">Module Completion</h3>
                <p className="text-blue-700 text-sm mb-4">I have watched the video and read the materials.</p>
                <button
                    onClick={() => onUpdateProgress(updateKey, 100)}
                    disabled={progress === 100}
                    className={`px-6 py-2 rounded-lg font-bold transition-colors ${progress === 100 ? 'bg-green-500 text-white cursor-default' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
                >
...
const TrainingGuide = ({ user, onUpdateProgress, onBack }: { user: User, onUpdateProgress: (mod: keyof User['progress'], val: number) => void, onBack?: () => void }) => {
    ...
    return (
        <>
            <TrainingSidebar />
            <div className="flex-1 h-screen overflow-y-auto bg-[#F7FAFC]">
                <div className="p-8 pb-32 max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                    {/* Middle Column: Module Nav */}
                    <div className="w-full lg:w-72 flex-shrink-0">
                        <div className="sticky top-8 bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6">
                            <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-6 px-1">ATMT1 – 5 WHYS</h3>
                            ...
                            <div className="mt-8 px-1 pt-6 border-t border-gray-50">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-[10px] font-bold text-gray-700 uppercase">Total Progress</span>
                                    <span className="text-xs font-bold text-blue-600">{calculateTotalProgress()}%</span>
                                </div>
                                <div className="w-full bg-[#EBF1FF] rounded-full h-1.5">
                                    <div className="bg-blue-600 h-1.5 rounded-full transition-all duration-500" style={{ width: `${calculateTotalProgress()}%` }}></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 pb-20">
                        <div className="space-y-16 animate-slide-up">
                            {/* Header Banner */}
                            <div className="bg-gradient-to-br from-[#1E40AF] via-[#2563EB] to-[#6366F1] rounded-2xl p-10 text-white shadow-xl mb-12 relative overflow-hidden">
                                <div className="relative z-10">
                                    <h1 className="text-4xl font-extrabold mb-3">Complete the Course</h1>
                                    <p className="text-blue-100 text-lg font-medium">Power Tool for Effective & Efficient Therapy</p>
                                </div>

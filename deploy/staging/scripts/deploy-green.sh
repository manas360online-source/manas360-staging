...
docker compose -f "$COMPOSE_FILE" pull "frontend_${TARGET_COLOR}" "backend_${TARGET_COLOR}" || true
docker compose -f "$COMPOSE_FILE" up -d --no-deps "frontend_${TARGET_COLOR}" "backend_${TARGET_COLOR}" nginx
